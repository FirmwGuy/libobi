...
typedef struct obi_math_blas_openblas_ctx_v0 {
    const obi_host_v0* host; /* borrowed */
} obi_math_blas_openblas_ctx_v0;
...
static CBLAS_LAYOUT _to_cblas_layout(obi_blas_layout_v0 layout) {
    return (layout == OBI_BLAS_ROW_MAJOR) ? CblasRowMajor : CblasColMajor;
}

static CBLAS_TRANSPOSE _to_cblas_transpose(obi_blas_transpose_v0 trans) {
...
static const obi_math_blas_api_v0 OBI_MATH_BLAS_OPENBLAS_API_V0 = {
...
static const char* _provider_id(void* ctx) {
    (void)ctx;
    return "obi.provider:math.blas.openblas";
}
...
static obi_status _get_profile(void* ctx,
    ...
    if (strcmp(profile_id, OBI_PROFILE_MATH_BLAS_V0) == 0) {
        ...
        p->api = &OBI_MATH_BLAS_OPENBLAS_API_V0;
        p->ctx = ctx;
        return OBI_STATUS_OK;
    }
...
static const char* _describe_json(void* ctx) {
    (void)ctx;
    return "{\"provider_id\":\"obi.provider:math.blas.openblas\",\"provider_version\":\"0.1.0\","
...
static obi_status _describe_legal_metadata(void* ctx,
    ...
    out_meta->effective_license.summary_utf8 =
        "Effective posture reflects module plus required OpenBLAS dependency";
...
static void _destroy(void* ctx) {
    obi_math_blas_openblas_ctx_v0* p = (obi_math_blas_openblas_ctx_v0*)ctx;
    if (!p) {
        return;
    }
...
static const obi_provider_api_v0 OBI_MATH_BLAS_OPENBLAS_PROVIDER_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static obi_status _create(const obi_host_v0* host, obi_provider_v0* out_provider) {
    ...
    obi_math_blas_openblas_ctx_v0* ctx = NULL;
    if (host->alloc) {
        ctx = (obi_math_blas_openblas_ctx_v0*)host->alloc(host->ctx, sizeof(*ctx));
    } else {
        ctx = (obi_math_blas_openblas_ctx_v0*)malloc(sizeof(*ctx));
    }
    if (!ctx) {
    ...
    out_provider->api = &OBI_MATH_BLAS_OPENBLAS_PROVIDER_API_V0;
...
OBI_EXPORT const obi_provider_factory_desc_v0 obi_provider_factory_v0 = {
    ...
    .reserved = 0u,
    .provider_id = "obi.provider:math.blas.openblas",
