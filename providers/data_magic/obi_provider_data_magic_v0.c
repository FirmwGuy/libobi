...
typedef struct obi_data_magic_ctx_v0 {
    const obi_host_v0* host; /* borrowed */
    magic_t mime_magic;
    magic_t desc_magic;
} obi_data_magic_ctx_v0;
...
static obi_status _fill_info(obi_data_magic_ctx_v0* p,
                             const void* bytes,
                             size_t size,
                             obi_file_type_info_v0* out_info) {
    if (!p || !bytes || size == 0u || !out_info) {
        return OBI_STATUS_BAD_ARG;
    }

    memset(out_info, 0, sizeof(*out_info));

    const char* mime = magic_buffer(p->mime_magic, bytes, size);
    const char* desc = magic_buffer(p->desc_magic, bytes, size);
    if (!mime || !desc) {
...
static obi_status _detect_from_reader(void* ctx,
    ...
    while (total < cap) {
        size_t n = 0u;
        obi_status st = reader.api->read(reader.ctx, (uint8_t*)buf + total, cap - total, &n);
        if (st != OBI_STATUS_OK) {
            free(buf);
            return st;
        }
    ...
    obi_status st = _fill_info((obi_data_magic_ctx_v0*)ctx, buf, total, out_info);
    free(buf);
    return st;
}

static const obi_data_file_type_api_v0 OBI_DATA_MAGIC_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
    .struct_size = (uint32_t)sizeof(obi_data_file_type_api_v0),
    .reserved = 0,
    .caps = OBI_FILE_TYPE_CAP_ENCODING,
...
static const char* _provider_id(void* ctx) {
    (void)ctx;
    return "obi.provider:data.magic";
}
...
static const char* _describe_json(void* ctx) {
    (void)ctx;
    return "{\"provider_id\":\"obi.provider:data.magic\",\"profiles\":[\"obi.profile:data.file_type-0\"]}";
}

static void _destroy(void* ctx) {
    obi_data_magic_ctx_v0* p = (obi_data_magic_ctx_v0*)ctx;
    if (!p) {
        return;
    }
...
static const obi_provider_api_v0 OBI_DATA_MAGIC_PROVIDER_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static obi_status _create(const obi_host_v0* host, obi_provider_v0* out_provider) {
    ...
    obi_data_magic_ctx_v0* ctx = NULL;
    if (host->alloc) {
        ctx = (obi_data_magic_ctx_v0*)host->alloc(host->ctx, sizeof(*ctx));
    } else {
        ctx = (obi_data_magic_ctx_v0*)malloc(sizeof(*ctx));
    }
    if (!ctx) {
        return OBI_STATUS_OUT_OF_MEMORY;
    }
    ...
    out_provider->api = &OBI_DATA_MAGIC_PROVIDER_API_V0;
...
OBI_EXPORT const obi_provider_factory_desc_v0 obi_provider_factory_v0 = {
    ...
    .reserved = 0,
    .provider_id = "obi.provider:data.magic",
