...
typedef struct obi_data_compression_libdeflate_ctx_v0 {
    const obi_host_v0* host; /* borrowed */
} obi_data_compression_libdeflate_ctx_v0;
...
static int _codec_supported(const char* codec_id) {
    ...
    return strcmp(codec_id, "deflate") == 0 || strcmp(codec_id, "zlib") == 0;
}
...
static obi_status _compression_compress(void* ctx,
    ...
    struct libdeflate_compressor* comp = libdeflate_alloc_compressor(_clamp_level(params));
    if (!comp) {
        free(in_data);
        return OBI_STATUS_OUT_OF_MEMORY;
    }

    size_t out_cap = libdeflate_zlib_compress_bound(comp, in_size);
    uint8_t* out_data = (uint8_t*)malloc(out_cap);
    if (!out_data) {
    ...
    size_t out_size = libdeflate_zlib_compress(comp, in_data, in_size, out_data, out_cap);
    libdeflate_free_compressor(comp);
    free(in_data);
    if (out_size == 0u) {
        free(out_data);
    ...
    st = _writer_write_all(dst, out_data, out_size);
    if (st == OBI_STATUS_OK) {
        if (out_bytes_in) {
...
static obi_status _compression_decompress(void* ctx,
    ...
    size_t out_size = 0u;
    enum libdeflate_result dr = LIBDEFLATE_SHORT_OUTPUT;

    while (dr == LIBDEFLATE_SHORT_OUTPUT) {
        ...
        dr = libdeflate_zlib_decompress(decomp,
                                        in_data,
                                        in_size,
                                        out_data,
                                        out_cap,
                                        &out_size);
        if (dr == LIBDEFLATE_SHORT_OUTPUT) {
    ...
    if (dr != LIBDEFLATE_SUCCESS) {
        free(out_data);
        return (dr == LIBDEFLATE_BAD_DATA) ? OBI_STATUS_BAD_ARG : OBI_STATUS_ERROR;
    }
...
static const obi_data_compression_api_v0 OBI_DATA_COMPRESSION_LIBDEFLATE_API_V0 = {
...
static const char* _provider_id(void* ctx) {
    (void)ctx;
    return "obi.provider:data.compression.libdeflate";
}
...
static obi_status _get_profile(void* ctx,
    ...
    if (strcmp(profile_id, OBI_PROFILE_DATA_COMPRESSION_V0) == 0) {
        ...
        p->api = &OBI_DATA_COMPRESSION_LIBDEFLATE_API_V0;
        p->ctx = ctx;
        return OBI_STATUS_OK;
    }
...
static const char* _describe_json(void* ctx) {
    (void)ctx;
    return "{\"provider_id\":\"obi.provider:data.compression.libdeflate\",\"provider_version\":\"0.1.0\"," \
...
static obi_status _describe_legal_metadata(void* ctx,
    ...
    out_meta->effective_license.summary_utf8 =
        "Effective posture reflects module plus required libdeflate dependency";
...
static void _destroy(void* ctx) {
    obi_data_compression_libdeflate_ctx_v0* p = (obi_data_compression_libdeflate_ctx_v0*)ctx;
    if (!p) {
        return;
    }
...
static const obi_provider_api_v0 OBI_DATA_COMPRESSION_LIBDEFLATE_PROVIDER_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static obi_status _create(const obi_host_v0* host, obi_provider_v0* out_provider) {
    ...
    obi_data_compression_libdeflate_ctx_v0* ctx = NULL;
    if (host->alloc) {
        ctx = (obi_data_compression_libdeflate_ctx_v0*)host->alloc(host->ctx, sizeof(*ctx));
    } else {
        ctx = (obi_data_compression_libdeflate_ctx_v0*)malloc(sizeof(*ctx));
    }
    if (!ctx) {
        return OBI_STATUS_OUT_OF_MEMORY;
    }
    ...
    out_provider->api = &OBI_DATA_COMPRESSION_LIBDEFLATE_PROVIDER_API_V0;
...
OBI_EXPORT const obi_provider_factory_desc_v0 obi_provider_factory_v0 = {
    ...
    .reserved = 0u,
    .provider_id = "obi.provider:data.compression.libdeflate",
