...
#define OBI_IPC_BUS_SDBUS_PROVIDER_ID "obi.provider:ipc.bus.sdbus"
#define OBI_IPC_BUS_SDBUS_PROVIDER_VERSION "0.1.0"
#define OBI_IPC_BUS_SDBUS_PROVIDER_SPDX "MPL-2.0"
#define OBI_IPC_BUS_SDBUS_PROVIDER_LICENSE_CLASS "weak_copyleft"
#define OBI_IPC_BUS_SDBUS_PROVIDER_DEPS_JSON \
    "[{\"name\":\"libsystemd\",\"version\":\"dynamic\",\"spdx_expression\":\"LGPL-2.1-or-later\",\"class\":\"weak_copyleft\"}]"

typedef struct obi_bus_signal_msg_sdbus_v0 {
    char* sender_name;
    char* object_path;
    char* interface_name;
    char* member_name;
    char* args_json;
} obi_bus_signal_msg_sdbus_v0;

typedef struct obi_bus_conn_sdbus_ctx_v0 obi_bus_conn_sdbus_ctx_v0;

typedef struct obi_bus_subscription_sdbus_ctx_v0 {
    obi_bus_conn_sdbus_ctx_v0* owner;
    ...
    obi_bus_signal_msg_sdbus_v0* queue;
    size_t queue_count;
    size_t queue_cap;
} obi_bus_subscription_sdbus_ctx_v0;

struct obi_bus_conn_sdbus_ctx_v0 {
    char* endpoint;
    char* requested_name;
    ...
    obi_bus_subscription_sdbus_ctx_v0** subscriptions;
    size_t subscription_count;
    size_t subscription_cap;
};

typedef struct obi_bus_reply_hold_sdbus_v0 {
    char* results_json;
    char* remote_error_name;
    char* error_details_json;
} obi_bus_reply_hold_sdbus_v0;

typedef struct obi_bus_signal_hold_sdbus_v0 {
    char* sender_name;
    char* object_path;
    char* interface_name;
    char* member_name;
    char* args_json;
} obi_bus_signal_hold_sdbus_v0;

typedef struct obi_ipc_bus_sdbus_ctx_v0 {
    const obi_host_v0* host;
} obi_ipc_bus_sdbus_ctx_v0;
...
static void _signal_msg_clear(obi_bus_signal_msg_sdbus_v0* msg) {
    if (!msg) {
        return;
    }
...
static int _subscription_matches(const obi_bus_subscription_sdbus_ctx_v0* sub,
                                 const obi_bus_signal_msg_sdbus_v0* msg) {
    if (!sub || !msg) {
        return 0;
    }
...
static int _subscription_queue_push(obi_bus_subscription_sdbus_ctx_v0* sub,
                                    const obi_bus_signal_msg_sdbus_v0* src) {
    if (!sub || !src) {
        return 0;
    }
    ...
    if (sub->queue_count == sub->queue_cap) {
        ...
        sub->queue = (obi_bus_signal_msg_sdbus_v0*)mem;
        sub->queue_cap = next_cap;
    }

    obi_bus_signal_msg_sdbus_v0* dst = &sub->queue[sub->queue_count];
...
static void _reply_release(void* release_ctx, obi_bus_reply_v0* out_reply) {
    obi_bus_reply_hold_sdbus_v0* hold = (obi_bus_reply_hold_sdbus_v0*)release_ctx;
    if (!hold) {
        return;
    }
...
static void _signal_release(void* release_ctx, obi_bus_signal_v0* out_signal) {
    obi_bus_signal_hold_sdbus_v0* hold = (obi_bus_signal_hold_sdbus_v0*)release_ctx;
    if (!hold) {
        return;
    }
...
static void _conn_detach_subscription(obi_bus_conn_sdbus_ctx_v0* conn,
                                      obi_bus_subscription_sdbus_ctx_v0* sub) {
...
static obi_status _subscription_next(void* ctx,
                                     ...
                                     bool* out_has_signal) {
    obi_bus_subscription_sdbus_ctx_v0* sub = (obi_bus_subscription_sdbus_ctx_v0*)ctx;
    ...
    *out_has_signal = false;
    ...
    if (sub->queue_count == 0u && timeout_ns > 0u) {
        ...
        while (sub->queue_count == 0u && waited < timeout_ns) {
            ...
            if (sub->owner && sub->owner->using_real_bus && sub->owner->bus) {
                int rc = sd_bus_process(sub->owner->bus, NULL);
                if (rc > 0) {
                    continue;
                }
            }
    ...
    obi_bus_signal_msg_sdbus_v0 msg = sub->queue[0];
    for (size_t i = 1u; i < sub->queue_count; i++) {
        sub->queue[i - 1u] = sub->queue[i];
    }
    sub->queue_count--;

    obi_bus_signal_hold_sdbus_v0* hold = (obi_bus_signal_hold_sdbus_v0*)calloc(1u, sizeof(*hold));
    if (!hold) {
...
static const obi_bus_subscription_api_v0 OBI_IPC_BUS_SDBUS_SUBSCRIPTION_API_V0 = {
...
static obi_status _conn_call_json(void* ctx,
    ...
    obi_bus_conn_sdbus_ctx_v0* conn = (obi_bus_conn_sdbus_ctx_v0*)ctx;
    ...
    if (conn->using_real_bus && conn->bus) {
        ...
        (void)sd_bus_call_method(conn->bus,
    ...
    obi_bus_reply_hold_sdbus_v0* hold = (obi_bus_reply_hold_sdbus_v0*)calloc(1u, sizeof(*hold));
    if (!hold) {
        return OBI_STATUS_OUT_OF_MEMORY;
    }
...
static obi_status _conn_subscribe_signals(void* ctx,
    ...
    obi_bus_subscription_sdbus_ctx_v0* sub =
        (obi_bus_subscription_sdbus_ctx_v0*)calloc(1u, sizeof(*sub));
    if (!sub) {
        return OBI_STATUS_OUT_OF_MEMORY;
    }
    ...
    if (conn->subscription_count == conn->subscription_cap) {
        ...
        conn->subscriptions = (obi_bus_subscription_sdbus_ctx_v0**)mem;
    ...
    out_subscription->api = &OBI_IPC_BUS_SDBUS_SUBSCRIPTION_API_V0;
...
static obi_status _conn_emit_signal_json(void* ctx, const obi_bus_signal_emit_v0* signal) {
    ...
    obi_bus_signal_msg_sdbus_v0 msg;
    memset(&msg, 0, sizeof(msg));
    ...
    if (conn->using_real_bus && conn->bus) {
        (void)sd_bus_emit_signal(conn->bus,
                                 msg.object_path,
                                 msg.interface_name,
                                 msg.member_name,
                                 "s",
                                 msg.args_json);
        (void)sd_bus_flush(conn->bus);
    }

    for (size_t i = 0u; i < conn->subscription_count; i++) {
        obi_bus_subscription_sdbus_ctx_v0* sub = conn->subscriptions[i];
...
static obi_status _conn_request_name(void* ctx,
    ...
    *out_acquired = true;
    if (conn->using_real_bus && conn->bus) {
        uint64_t req_flags = 0u;
        if ((flags & OBI_BUS_REQUEST_NAME_REPLACE_EXISTING) != 0u) {
            req_flags |= SD_BUS_NAME_REPLACE_EXISTING;
        }
        if ((flags & OBI_BUS_REQUEST_NAME_DO_NOT_QUEUE) == 0u) {
            req_flags |= SD_BUS_NAME_QUEUE;
        }

        int rc = sd_bus_request_name(conn->bus, name_c, req_flags);
        if (rc < 0) {
            free(name_c);
            *out_acquired = false;
...
static obi_status _conn_release_name(void* ctx, obi_utf8_view_v0 name) {
    ...
    if (conn->using_real_bus && conn->bus) {
        ...
        int rc = sd_bus_release_name(conn->bus, name_c);
        free(name_c);
        if (rc < 0) {
            return OBI_STATUS_UNAVAILABLE;
        }
...
static void _conn_destroy(void* ctx) {
    ...
    while (conn->subscription_count > 0u) {
        obi_bus_subscription_sdbus_ctx_v0* sub = conn->subscriptions[conn->subscription_count - 1u];
        conn->subscription_count--;
        if (!sub) {
            continue;
        }
    ...
    if (conn->bus) {
        (void)sd_bus_flush(conn->bus);
        conn->bus = sd_bus_unref(conn->bus);
    }

    free(conn);
}

static const obi_bus_conn_api_v0 OBI_IPC_BUS_SDBUS_CONN_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static int _connect_backend(const obi_bus_connect_params_v0* params, sd_bus** out_bus) {
    sd_bus* bus = NULL;
    int rc = -1;
    ...
    switch ((obi_bus_endpoint_kind_v0)params->endpoint_kind) {
        ...
        case OBI_BUS_ENDPOINT_CUSTOM: {
            ...
            rc = sd_bus_new(&bus);
            if (rc >= 0) {
                rc = sd_bus_set_address(bus, address);
            }
            if (rc >= 0) {
                rc = sd_bus_start(bus);
            }
...
static obi_status _ipc_bus_connect(void* ctx,
    ...
    obi_bus_conn_sdbus_ctx_v0* conn = (obi_bus_conn_sdbus_ctx_v0*)calloc(1u, sizeof(*conn));
    if (!conn) {
        return OBI_STATUS_OUT_OF_MEMORY;
    }
    ...
    out_conn->api = &OBI_IPC_BUS_SDBUS_CONN_API_V0;
    out_conn->ctx = conn;
    return OBI_STATUS_OK;
}

static const obi_ipc_bus_api_v0 OBI_IPC_BUS_SDBUS_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static const char* _provider_id(void* ctx) {
    (void)ctx;
    return OBI_IPC_BUS_SDBUS_PROVIDER_ID;
}

static const char* _provider_version(void* ctx) {
    (void)ctx;
    return OBI_IPC_BUS_SDBUS_PROVIDER_VERSION;
}
...
static obi_status _get_profile(void* ctx,
    ...
    if (strcmp(profile_id, OBI_PROFILE_IPC_BUS_V0) == 0) {
        ...
        p->api = &OBI_IPC_BUS_SDBUS_API_V0;
        p->ctx = ctx;
        return OBI_STATUS_OK;
    }
...
static const char* _describe_json(void* ctx) {
    (void)ctx;
    return "{\"provider_id\":\"" OBI_IPC_BUS_SDBUS_PROVIDER_ID "\","
           "\"provider_version\":\"" OBI_IPC_BUS_SDBUS_PROVIDER_VERSION "\","
           "\"profiles\":[\"obi.profile:ipc.bus-0\"],"
           "\"license\":{\"spdx_expression\":\"" OBI_IPC_BUS_SDBUS_PROVIDER_SPDX "\",\"class\":\""
           OBI_IPC_BUS_SDBUS_PROVIDER_LICENSE_CLASS "\"},"
...
static const obi_provider_api_v0 OBI_PROVIDER_IPC_BUS_SDBUS_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static obi_status _create(const obi_host_v0* host, obi_provider_v0* out_provider) {
    ...
    obi_ipc_bus_sdbus_ctx_v0* ctx = NULL;
    if (host->alloc) {
        ctx = (obi_ipc_bus_sdbus_ctx_v0*)host->alloc(host->ctx, sizeof(*ctx));
    } else {
        ctx = (obi_ipc_bus_sdbus_ctx_v0*)calloc(1u, sizeof(*ctx));
    }
    if (!ctx) {
        return OBI_STATUS_OUT_OF_MEMORY;
    }
    ...
    out_provider->api = &OBI_PROVIDER_IPC_BUS_SDBUS_API_V0;
...
OBI_EXPORT const obi_provider_factory_desc_v0 obi_provider_factory_v0 = {
    ...
    .reserved = 0u,
    .provider_id = OBI_IPC_BUS_SDBUS_PROVIDER_ID,
    .provider_version = OBI_IPC_BUS_SDBUS_PROVIDER_VERSION,
    .create = _create,
};
