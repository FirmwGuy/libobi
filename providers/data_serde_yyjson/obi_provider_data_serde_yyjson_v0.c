...
typedef struct obi_data_serde_yyjson_ctx_v0 {
    const obi_host_v0* host; /* borrowed */
} obi_data_serde_yyjson_ctx_v0;
...
static int _yyjson_push_value(yyjson_val* v,
    ...
    if (yyjson_is_obj(v)) {
        ...
        yyjson_obj_foreach(v, idx, max, key, val) {
            const char* key_s = yyjson_get_str(key);
            size_t key_n = key_s ? strlen(key_s) : 0u;
            if (!_serde_push_event(io_events, io_count, io_cap, OBI_SERDE_EVENT_KEY, key_s ? key_s : "", key_n, 0u)) {
                _serde_set_error(out_err, "out of memory emitting KEY");
                return 0;
            }

            if (!_yyjson_push_value(val, io_events, io_count, io_cap, out_err, depth + 1)) {
                return 0;
            }
        }
    ...
    if (yyjson_is_num(v)) {
        ...
        if (!raw || raw[0] == '\0') {
            char num_buf[64];
            if (yyjson_is_sint(v)) {
                (void)snprintf(num_buf, sizeof(num_buf), "%lld", (long long)yyjson_get_sint(v));
            } else if (yyjson_is_uint(v)) {
                (void)snprintf(num_buf, sizeof(num_buf), "%llu", (unsigned long long)yyjson_get_uint(v));
            } else {
                (void)snprintf(num_buf, sizeof(num_buf), "%.17g", yyjson_get_real(v));
            }
        ...
        if (!_serde_push_event(io_events, io_count, io_cap, OBI_SERDE_EVENT_NUMBER, raw, strlen(raw), 0u)) {
            _serde_set_error(out_err, "out of memory emitting NUMBER");
            return 0;
        }
        return 1;
...
static obi_status _serde_parse_json_bytes(const uint8_t* bytes,
    ...
    yyjson_doc* doc = yyjson_read_opts((char*)(void*)bytes, size, 0u, NULL, &err);
    if (!doc) {
        return OBI_STATUS_BAD_ARG;
    }
    ...
    if (!root || !_yyjson_push_value(root, &p->events, &p->count, &p->cap, &p->last_error, 0)) {
        if (!p->last_error) {
...
static const obi_serde_parser_api_v0 OBI_DATA_SERDE_YYJSON_PARSER_API_V0 = {
...
static obi_status _serde_events_open_reader(void* ctx,
    ...
    out_parser->api = &OBI_DATA_SERDE_YYJSON_PARSER_API_V0;
    return OBI_STATUS_OK;
}
...
static const obi_data_serde_events_api_v0 OBI_DATA_SERDE_YYJSON_EVENTS_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static const obi_serde_emitter_api_v0 OBI_DATA_SERDE_YYJSON_EMITTER_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static obi_status _serde_emit_open_params_validate(const obi_serde_emit_open_params_v0* params) {
...
static obi_status _serde_emit_open_writer(void* ctx,
    ...
    (void)ctx;
    if (!out_emitter || !writer.api || !writer.api->write) {
        return OBI_STATUS_BAD_ARG;
    }
    obi_status params_st = _serde_emit_open_params_validate(params);
    if (params_st != OBI_STATUS_OK) {
        return params_st;
    }
    ...
    out_emitter->api = &OBI_DATA_SERDE_YYJSON_EMITTER_API_V0;
    out_emitter->ctx = e;
    return OBI_STATUS_OK;
}

static const obi_data_serde_emit_api_v0 OBI_DATA_SERDE_YYJSON_EMIT_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static const char* _provider_id(void* ctx) {
    (void)ctx;
    return "obi.provider:data.serde.yyjson";
}
...
static obi_status _get_profile(void* ctx,
    ...
    if (strcmp(profile_id, OBI_PROFILE_DATA_SERDE_EVENTS_V0) == 0) {
        ...
        p->api = &OBI_DATA_SERDE_YYJSON_EVENTS_API_V0;
        p->ctx = ctx;
        return OBI_STATUS_OK;
    }
    ...
    if (strcmp(profile_id, OBI_PROFILE_DATA_SERDE_EMIT_V0) == 0) {
        ...
        p->api = &OBI_DATA_SERDE_YYJSON_EMIT_API_V0;
        p->ctx = ctx;
        return OBI_STATUS_OK;
    }
...
static const char* _describe_json(void* ctx) {
    (void)ctx;
    return "{\"provider_id\":\"obi.provider:data.serde.yyjson\",\"provider_version\":\"0.1.0\"," \
...
static obi_status _describe_legal_metadata(void* ctx,
    ...
    out_meta->effective_license.summary_utf8 =
        "Effective posture reflects module plus required yyjson dependency";
...
static void _destroy(void* ctx) {
    obi_data_serde_yyjson_ctx_v0* p = (obi_data_serde_yyjson_ctx_v0*)ctx;
    if (!p) {
        return;
    }
...
static const obi_provider_api_v0 OBI_DATA_SERDE_YYJSON_PROVIDER_API_V0 = {
    .abi_major = OBI_CORE_ABI_MAJOR,
    .abi_minor = OBI_CORE_ABI_MINOR,
...
static obi_status _create(const obi_host_v0* host, obi_provider_v0* out_provider) {
    ...
    obi_data_serde_yyjson_ctx_v0* ctx = NULL;
    if (host->alloc) {
        ctx = (obi_data_serde_yyjson_ctx_v0*)host->alloc(host->ctx, sizeof(*ctx));
    } else {
        ctx = (obi_data_serde_yyjson_ctx_v0*)malloc(sizeof(*ctx));
    }
    if (!ctx) {
        return OBI_STATUS_OUT_OF_MEMORY;
    }
    ...
    out_provider->api = &OBI_DATA_SERDE_YYJSON_PROVIDER_API_V0;
...
OBI_EXPORT const obi_provider_factory_desc_v0 obi_provider_factory_v0 = {
    ...
    .reserved = 0u,
    .provider_id = "obi.provider:data.serde.yyjson",
